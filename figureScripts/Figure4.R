...
setwd('~/Figures/')
seurat <- Load10X_Spatial('~/DiffuseMidlineGliomaA1/outs/')
...
# Figure 4A
estimateOutput <- read.csv('~/DiffuseMidlineGliomaA1_estimateOutput.csv', row.names = 1)
...
cluster <- read.csv('~/DiffuseMidlineGliomaA1/outs/analysis/clustering/graphclust/clusters.csv',row.names=1) #10X graph clusters
...
boxplot <- ggplot(minidf,aes(x=as.factor(Cluster),y=StromalScore,color=as.factor(Cluster))) + 
    ...
    geom_jitter(aes(color = as.factor(Cluster)), alpha = 0.2) + #points
    geom_boxplot(alpha=0.8,outlier.shape=NA) +
    stat_summary(fun.y=mean, geom="point", size=2) +
    stat_summary(fun.data = get_box_stats, geom = "text", hjust = 0.5, vjust = 0.5,size=3.2,color='black',family='Arial Narrow',fontface='bold') +
