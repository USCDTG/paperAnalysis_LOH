...
# Functions below are the updated functions used in the LOH identification approach
...
importVCF <- function(vcf){
    ...
    intermediateDFList <- lapply(sampleClusters, function(x){
        ...
        return(intermediate)})
    save <- intermediateDFList
    intermediateDFList <- lapply(save, function(x) x[rowSums(is.na(x)) != ncol(x), ])
...
runHMM_1 <- function(dataframeList, initProbs, trProbs){
    ...
    for(i in 1:length(dataframeList)){
        ...
        initProb1 <- initProbs[CHRvalue,1] 
        initProb2 <- initProbs[CHRvalue,2]
...
annotateRegions <- function(hmmOutput){
    hmmOutput$regionScoreThreshold <- (hmmOutput$segment_nSNPs * 0.85) * 0.5
    d <- dplyr::mutate(.data = hmmOutput, segment_State = dplyr::case_when(state == 'errored' ~ 'Errored',
...
hiddenMarkovAnalysis2 <- function(df, initProbs, trProbs){
    ...
    done <- annotateRegions(done)
    done <- done %>% filter(segment_State != 'Errored')
    done <- unique(done)
    return(done)
}

plotAlleleFractions <- function(data){
    ggplot(data %>% filter(segment_State != 'Errored'),aes(x=position,
                      y=alleleFraction,
                      color=segment_State)) + 
...
plotSegments <- function(data, listToHighlight){
    ...
    if(missing(listToHighlight)) {
        x <- default
        ggplot(data %>% filter(segment_State != 'Errored')) + 
            ...
            facet_grid(cluster~as.numeric(chromosome), switch = 'x', scales = 'free') +
            scale_color_manual(values = c('blue','goldenrod','grey'), labels = c('Heterozygous','LOH','Undefined')) +
            xlab("Chromosome") +
            ylab("") +
            labs(color = c('State')) +
