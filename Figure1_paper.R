...
for(i in 1:length(samples)){
    a <- Load10X_Spatial(paste0('/Volumes/GoogleDrive/Shared drives/10X_Alpha_Test_Phase_1_and_2/paper2023/rawData/spatial/',samples[[i]],'/outs'))
    a@meta.data$sample <- samples[[i]]
    a[["pctMito"]] <- PercentageFeatureSet(a, pattern = "^MT-")
    listOfSamples[[i]] <- a
}

additionalSample <- Load10X_Spatial(paste0('/Volumes/GoogleDrive/Shared drives/10X_Alpha_Test_Phase_1_and_2/paper2023/rawData/spatial/FF_D1'))
...
for(i in 1:length(listD)){
    ...
    ggsave(s,filename = paste0('/Users/michelgw/Desktop/',samples[[i]],'_integratedClusters.png'))
}
...
raviTranscriptionalModules <- read.csv('/Users/michelgw/FourModules.tsv', sep='\t')
...
png(file="testing_GBM_niche.png", width = 650, height = 650)
...
# Figure 1G
renRegionalPrograms <- read.csv('/Users/michelgw/markerSet.tsv',sep='\t')
...
png(file="GBM_regionalPrograms.png", width = 800, height = 800)
pushViewport(viewport(gp = gpar(fontfamily = "Arial Narrow")))
draw(ht, padding = unit(c(2, 2, 2, 40), "mm"))
dev.off()
